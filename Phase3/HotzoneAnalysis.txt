...
object HotzoneAnalysis {
  ...
  def runHotZoneAnalysis(spark: SparkSession, pointPath: String, rectanglePath: String): DataFrame = {
    ...
    joinDf.printSchema()
    val outputDf = joinDf.groupBy("rectangle").count().sort(asc("rectangle")).coalesce(1)
    outputDf.printSchema()

    //joinDf.groupBy("rectangle").count().sort(asc("rectangle")).show(20)
    //return joinDf.groupBy("rectangle").count() // YOU NEED TO CHANGE THIS PART
    return outputDf
  }
